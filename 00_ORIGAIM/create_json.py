...
def main():
    ...
    basedir = Path(
        "/storage/workspaces/artorg_msb/hpc_abaqus/poncioni/HFE/00_ORIGAIM/REPRO/"
    )
    ...
    for file in basedir.rglob("*.AIM"):  # Use rglob to find files recursively
        ...
        names_dict[names] = str(
            file.parent.relative_to(basedir)
        )  # Use file.parent.relative_to(basedir) to get the relative directory path
